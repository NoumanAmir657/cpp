...
template <typename Derived> class AnimalBase {
public:
...
public:
  ...
  void makeNoiseImpl() const { std::cout << "Dog says Woof!\n"; }
};

class Cat : public AnimalBase<Cat> {
public:
  // This is the specific implementation the CRTP base calls.
  void makeNoiseImpl() const { std::cout << "Cat says Meow!\n"; }
};

// This function is the site of polymorphism.
// It accepts any type T that inherits from AnimalBase<T>.
template <typename T> void communicate(const AnimalBase<T> &animal) {
...
int main() {
  ...
  // Dog and Cat have different, unrelated base classes (AnimalBase<Dog> and
  // AnimalBase<Cat>). The compiler will reject this attempt to store
  // heterogeneous types. std::vector<AnimalBase<?>> animals = { myDog, myCat };
}
