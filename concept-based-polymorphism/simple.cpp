...
template <typename ConcreteAnimal> struct AnimalModel : public AnimalConcept {
  ...
  void makeNoise() const override { animal.makeNoise(); }
};
...
public:
  ...
  template <typename T>
  Animal(T animal)
      : pImpl(std::make_unique<AnimalModel<T>>(std::move(animal))) {}
...
int main() {
  // We can put different concrete types into a single container of the wrapper
  // type.
