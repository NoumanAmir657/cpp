...
struct DrawableTraits {
  ...
  template <typename T> struct Model : public DrawableConcept {
    T item;
  ...
  template <> struct Model<ExternalCircle> : public DrawableConcept {
    ...
    void draw() const override { circle.render(); }
  };

  // Explicit Specialization 2: Model for ExternalSquare
  // This Model adapts the external type to our Concept.
  template <> struct Model<ExternalSquare> : public DrawableConcept {
    ExternalSquare square;
    Model(ExternalSquare s) : square(std::move(s)) {}

    // Implementation of the Concept method:
    // It calls the externally named method 'render()'.
    void draw() const override { square.render(); }
  };
};
...
public:
  ...
  template <typename T> Drawable(T item) {
...
// NOTE: For simplicity, we are assuming the Generic Model is NOT used for
// external types due to the name mismatch. In a real scenario, you'd use C++20
// Concepts to constrain the generic model to only types that have 'draw()'.
struct InternalTriangle {
  void draw() const {
    std::cout << "Drawing a Triangle (Internal, Conforming).\n";
  }
};
